...
def render_owner_employee_accounts():
    """Render owner employee account management page."""
    st.markdown("<div class='app-header'><h1>Employee Accounts</h1><p>Create, view, and delete employee logins</p></div>", unsafe_allow_html=True)
...
def main():
    ...
    if not st.session_state["authenticated"]:
        ...
        if page == "dashboard":
            ...
            if user.is_owner():
                render_owner_employee_accounts()
            else:
                st.error("Access denied.")
